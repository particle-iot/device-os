...
/**
 * Maximum length of a log handler ID.
 */
#define LOG_CONFIG_MAX_HANDLER_ID_SIZE 16
...
typedef enum log_config_command {
    ...
    LOG_CONFIG_LIST_HANDLERS = 3 ///< Get the list of active log handlers.
} log_config_command;
 ...
 */
typedef struct log_config_serial_param {
    uint8_t index; ///< Interface index.
    uint32_t baud_rate; ///< Baud rate.
} log_config_serial_param;
...
typedef struct log_filter {
    ...
    uint8_t level; ///< Logging level (one of the values defined the `LogLevel` enum).
} log_filter;
...
typedef struct log_config_add_handler_command {
    ...
    uint8_t level; ///< Default logging level (one of the values defined the `LogLevel` enum).
    uint8_t filter_count; ///< Number of elements in the `filters` array.
    const char* id; ///< Handler ID.
    const void* param; ///< Handler-specific parameters.
...
/**
 * Parameters of the `LOG_CONFIG_LIST_HANDLERS` command.
 */
typedef struct log_config_list_handlers_command {
    uint8_t version; ///< API version number.
    uint8_t reserved1;
    uint16_t reserved2;
} log_config_list_handlers_command;

/**
 * An element of a log handler list.
 */
typedef struct log_config_handler_list_item {
    char id[LOG_CONFIG_MAX_HANDLER_ID_SIZE]; ///< Handler ID.
} log_config_handler_list_item;

/**
 * Result of a `LOG_CONFIG_LIST_HANDLERS` command.
 */
typedef struct log_config_list_handlers_result {
...
} log_config_list_handlers_result;

/**
 * Logging configuration callback.
 *
 * @param command Configuration command (one of the values defined by the `log_config_command` enum).
 * @param command_data Command data.
 * @param command_result Command result.
 * @param user_data User data.
 */
typedef int(*log_config_callback)(int command, void* command_data, void** command_result, void* user_data);
...
/**
 * Set the logging configuration callback.
 *
...
// This function invokes the logging configuration callback and is internal to the system module
int log_config(int command, void* command_data, void** command_result);
