...
class SocketUsartBase : public Usart
{
    ...
    protected:
        ...
        void fillFromSocketIfNeeded() {
            ...
            if (socket!=SOCKET_INVALID && space>0) {
                sock_recv(socket, rx->buffer+rx->head, space, 0);
            }
        }
    ...
    public:
        ...
        virtual uint32_t write(uint8_t byte) override {
            if (!initSocket())
                return 0;
            return sock_send(socket, &byte, 1, 0);
        }
};
...
class SocketUsartClient : public SocketUsartBase {
    ...
    virtual bool initSocket() {
        if (socket==SOCKET_INVALID) {
            ...
            socketAddr.sin_port = htons(54);
            socketAddr.sin_addr.s_addr = htonl(INADDR_LOOPBACK);

            testResult = sock_connect(socket, (const sockaddr*)&socketAddr, sizeof(socketAddr));
            if (testResult) {
                sock_close(socket);
