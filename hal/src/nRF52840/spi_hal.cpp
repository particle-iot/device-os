...
static nrf5x_spi_info_t m_spi_map[TOTAL_SPI] = {
    {&m_spim3, NULL, APP_IRQ_PRIORITY_HIGH, PIN_INVALID, SCK, MOSI,
     MISO}, // TODO: SPI3 doesn't support SPI slave mode
...
static void spi_master_event_handler(nrfx_spim_evt_t const* p_event, void* p_context)
{
    ...
    {
        // LOG_DEBUG(TRACE, ">> spi: rx: %d, tx: %d", p_event->xfer_desc.tx_length,
...
void spi_slave_event_handler(nrfx_spis_evt_t const* p_event, void* p_context)
{
    ...
    if (p_event->evt_type == NRFX_SPIS_XFER_DONE)
    {
        ...
        SPARK_ASSERT(nrfx_spis_buffers_set(
                         m_spi_map[spi].slave, (uint8_t*)m_spi_map[spi].slave_tx_buf,
                         m_spi_map[spi].slave_buf_length, (uint8_t*)m_spi_map[spi].slave_rx_buf,
...
static void spi_init(HAL_SPI_Interface spi, SPI_Mode mode)
{
    ...
    {
        static const nrf_spim_mode_t nrf_spim_mode[4] = {NRF_SPIM_MODE_0, NRF_SPIM_MODE_1,
                                                         NRF_SPIM_MODE_2, NRF_SPIM_MODE_3};
        ...
        spim_config.bit_order = (m_spi_map[spi].bit_order == MSBFIRST)
                                    ? NRF_SPIM_BIT_ORDER_MSB_FIRST
                                    : NRF_SPIM_BIT_ORDER_LSB_FIRST;

        err_code = nrfx_spim_init(m_spi_map[spi].master, &spim_config, spi_master_event_handler,
                                  (void*)((int)spi));
    ...
    {
        static const nrf_spis_mode_t nrf_spis_mode[4] = {NRF_SPIS_MODE_0, NRF_SPIS_MODE_1,
                                                         NRF_SPIS_MODE_2, NRF_SPIS_MODE_3};
        ...
        spis_config.bit_order = (m_spi_map[spi].bit_order == MSBFIRST)
                                    ? NRF_SPIS_BIT_ORDER_MSB_FIRST
                                    : NRF_SPIS_BIT_ORDER_LSB_FIRST;

        err_code = nrfx_spis_init(m_spi_map[spi].slave, &spis_config, spi_slave_event_handler,
                                  (void*)((int)spi));
        SPARK_ASSERT(err_code == NRF_SUCCESS);

        HAL_Pin_Mode(m_spi_map[spi].ss_pin, INPUT_PULLUP);
        HAL_Interrupts_Attach(m_spi_map[spi].ss_pin, &HAL_SPI_SS_Handler, (void*)(spi), CHANGE,
                              NULL);
    }
...
void HAL_SPI_Info(HAL_SPI_Interface spi, hal_spi_info_t* info, void* reserved)
{
    ...
    if (info->version >= HAL_SPI_INFO_VERSION_1)
    {
        ...
        if (m_spi_map[spi].enabled)
        {
            ...
            {
            case SPI_CLOCK_DIV2:
                info->clock = info->system_clock / 2;
                break;
            case SPI_CLOCK_DIV4:
                info->clock = info->system_clock / 4;
                break;
            case SPI_CLOCK_DIV8:
                info->clock = info->system_clock / 8;
                break;
            case SPI_CLOCK_DIV16:
                info->clock = info->system_clock / 16;
                break;
            case SPI_CLOCK_DIV32:
                info->clock = info->system_clock / 32;
                break;
            case SPI_CLOCK_DIV64:
                info->clock = info->system_clock / 64;
                break;
            case SPI_CLOCK_DIV128:
                info->clock = info->system_clock / 128;
                break;
            case SPI_CLOCK_DIV256:
                info->clock = info->system_clock / 256;
                break;
            default:
                info->clock = 0;
            }
...
void HAL_SPI_DMA_Transfer(HAL_SPI_Interface spi, void* tx_buffer, void* rx_buffer, uint32_t length,
                          HAL_SPI_DMA_UserCallback userCallback)
{
    HAL_SPI_Acquire(spi, nullptr);
    ...
    else
    {
        ...
        uint32_t err_code = nrfx_spis_buffers_set(
            m_spi_map[spi].slave, (uint8_t*)m_spi_map[spi].slave_tx_buf,
            m_spi_map[spi].slave_buf_length, (uint8_t*)m_spi_map[spi].slave_rx_buf,
...
int32_t HAL_SPI_Set_Settings(HAL_SPI_Interface spi, uint8_t set_default, uint8_t clockdiv,
                             uint8_t order, uint8_t mode, void* reserved)
{
    HAL_SPI_Acquire(spi, nullptr);
