...
boost::array<ip::tcp::socket, SOCKET_COUNT> tcp_handles = {
    ip::tcp::socket(device_io_service), ip::tcp::socket(device_io_service),
    ip::tcp::socket(device_io_service), ip::tcp::socket(device_io_service),
    ip::tcp::socket(device_io_service), ip::tcp::socket(device_io_service),
    ip::tcp::socket(device_io_service), ip::tcp::socket(device_io_service)};

boost::array<ip::udp::socket, SOCKET_COUNT> udp_handles = {
    ip::udp::socket(device_io_service), ip::udp::socket(device_io_service),
    ip::udp::socket(device_io_service), ip::udp::socket(device_io_service),
    ip::udp::socket(device_io_service), ip::udp::socket(device_io_service),
    ip::udp::socket(device_io_service), ip::udp::socket(device_io_service)};
...
public:
    TCPServer(uint16_t port)
        : _port(port), acceptor(device_io_service, ip::tcp::endpoint(ip::tcp::v4(), port))
    {
    ...
    if (_timeout || available)
    {
        ...
        if (ec.value())
        {
            if (ec.value() == boost::system::errc::resource_deadlock_would_occur || // EDEADLK (35)
                ec.value() == boost::system::errc::resource_unavailable_try_again)
            {               // EAGAIN (11)
                result = 0; // No data available
            }
            else
            {
                DEBUG("socket receive error: %d %s, read=%d", ec.value(), ec.message().c_str(),
                      available);
            }
        }
    }
...
sock_result_t socket_receivefrom(sock_handle_t sock, void* buffer, socklen_t bufLen, uint32_t flags,
                                 sockaddr_t* addr, socklen_t* addrsize)
{
...
sock_result_t socket_sendto(sock_handle_t sd, const void* buffer, socklen_t len, uint32_t flags,
                            sockaddr_t* addr, socklen_t addr_size)
{
    unsigned port = addr->sa_data[0] << 8 | addr->sa_data[1];
    // 2-5 are IP address in network byte order
    const uint8_t* dest = addr->sa_data + 2;
...
sock_result_t socket_join_multicast(const HAL_IPAddress* addr, network_interface_t nif,
...
sock_result_t socket_leave_multicast(const HAL_IPAddress* addr, network_interface_t nif,
                                     socket_multicast_info_t* reserved)
{
    return -1;
}
