...
STM32_CAN_Info   CAN_MAP[TOTAL_CAN] =
{
    ...
    */
#if PLATFORM_ID == 6
   { CAN2, &RCC->APB1ENR, (RCC_APB1Periph_CAN1 | RCC_APB1Periph_CAN2), CAN2_TX_IRQn, CAN2_RX0_IRQn, CAN2_RX1_IRQn, CAN2_SCE_IRQn, D1, D2, GPIO_PinSource6, GPIO_PinSource5, GPIO_AF_CAN2 }  // CAN 2
#endif
};
...
inline void store_message(CanRxMsg        *pmessage,
                          CAN_Ring_Buffer *buffer)
{
   unsigned   len;
   ...
   if (i != buffer->tail)
   {
      memset((void *)&(buffer->buffer[buffer->head]), 0, sizeof(CAN_Message_Struct));
      ...
      buffer->buffer[buffer->head].Len = (pmessage->DLC <= 8) ? pmessage->DLC : 8;
      buffer->buffer[buffer->head].rtr = (pmessage->RTR & CAN_RTR_Remote) ? true : false;

      for (len = 0U; len < buffer->buffer[buffer->head].Len; len++)
      {
         buffer->buffer[buffer->head].Data[len] = pmessage->Data[len];
      }
...
/*******************************************************************************
   Name:           HAL_CAN_Init
   Description:    Initializes the CAN data objects
...
/*******************************************************************************
   Name:           HAL_CAN_Write_Data
   Description:    Sends a CAN message

   Parameters:
       @param channel CAN Channel (CAN1, CAN2, etc)
          @param pmessage Pointer to the CAN message
          @return 1 or -1.  1 success, -1 fail
...
/*******************************************************************************
   Name:           HAL_CAN_Available_Data
   Description:    returns the number of messages pending in the buffer.
...
/*******************************************************************************
   Name:           HAL_CAN_Read_Data
   Description:    Reads a CAN message from the buffer

   Parameters:
       @param channel CAN Channel (CAN1, CAN2, etc)
          @param pmessage pointer to where the message will be returned
          @return 1 or -1, 1 = success, -1 = fail.
                          ...
                          CAN_Message_Struct *pmessage)
{
   ...
   if ((canMap[channel]->can_rx_buffer->head == canMap[channel]->can_rx_buffer->tail) || (NULL == pmessage))
   {
      return -1;
   }
   else
...
/*******************************************************************************
   Name:           HAL_CAN_Peek_Data
   Description:    returns a CAN message without removing it from the buffer

   Parameters:
       @param channel CAN Channel (CAN1, CAN2, etc)
          @param pmessage pointer to where the message will get returned
          @return 1 or -1, 1 = success, -1 = fail
...
/*******************************************************************************
   Name:           HAL_CAN_Flush_Data
   Description:    Flushes the transmit buffer.  (NOT IMPLMENTED)
   ...
   if (CAN_GetITStatus(canMap[channel]->can_peripheral, CAN_IT_TME) != RESET)
   {
      ...
      do
      {
         ...
         if (canMap[channel]->can_tx_buffer->head == canMap[channel]->can_tx_buffer->tail)
         {
            ...
            // Turn off transmitting because buffer is empty.
            canMap[channel]->can_transmitting = false;
...
/*******************************************************************************
   Name:           HAL_CAN_Rx1_Handler
   Description:    CAN FIFO1 Rx Interrupt
