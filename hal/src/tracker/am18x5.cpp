...
int Am18x5::begin() {
    ...
    if (!initialized_) {
        ...
        if (config_.wdi_pin != PIN_INVALID) {
            HAL_Pin_Mode(config_.wdi_pin, OUTPUT);
            HAL_GPIO_Write(config_.wdi_pin, 1);
        }
        if (config_.int_pin != PIN_INVALID) {
            HAL_Pin_Mode(config_.int_pin, INPUT_PULLUP);
            HAL_InterruptExtraConfiguration extra = {};
            extra.version = HAL_INTERRUPT_EXTRA_CONFIGURATION_VERSION_1;
            CHECK(HAL_Interrupts_Attach(config_.int_pin, exRtcInterruptHandler, this, FALLING, &extra));
...
int Am18x5::end() {
    ...
    if (config_.wdi_pin != PIN_INVALID) {
        HAL_Pin_Mode(config_.wdi_pin, INPUT);
    }
    if (config_.int_pin != PIN_INVALID) {
        ...
        exRtcWorkerThread_ = nullptr;
        exRtcWorkerSemaphore_ = nullptr;
        HAL_Interrupts_Detach(config_.int_pin);
        HAL_Pin_Mode(config_.int_pin, INPUT);
    }
    if (initialized_) {
        ret = reset();
    }
