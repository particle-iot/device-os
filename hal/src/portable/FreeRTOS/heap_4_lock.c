...
BlockLink_t *pxLink;
    ...
    if( pv != NULL )
    {
        ...
        configASSERT( ((pv > ucHeap) && (pv < ucHeapEnd)) );
...
size_t uxAddress;
size_t xTotalHeapSize = xPortGetHeapSize();
...
size_t xPortGetHeapSize ( void )
{
    configASSERT( ( ucHeapEnd > ucHeap ) );
    return ucHeapEnd - ucHeap;
}
...
size_t xPortGetBlockSize ( void* ptr )
{
    ...
    configASSERT( ((ptr > ucHeap) && (ptr < ucHeapEnd)) );

    BlockLink_t* b = (BlockLink_t*) (((uint8_t*)ptr) - xHeapStructSize);
    return b->xBlockSize;
}
