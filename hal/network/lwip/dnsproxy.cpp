...
#if DEBUG_DNS64
#define DEBUG(...) LOG(TRACE, __VA_ARGS__)
#else
#define DEBUG(...)
#endif
...
int Dns::init(if_t iface, const ip6_addr_t& prefix, uint16_t port) {
    ...
    // Create a socket
    ctx_->sock = CHECK_SOCKET(sock_socket(AF_INET, SOCK_DGRAM, 0));
    // Set socket options
    int opt = 1;
...
int Dns::processQuery(char* data, size_t size, const sockaddr_in6& srcAddr) {
...
int Dns::parseQuery(char* data, size_t size, Query* q, const char** name) {
    ...
    if (q->q.qtype == Type::TYPE_ANY) {
        q->q.qtype = Type::A; // Prefer IPv4 by default
...
int Dns::sendResponse(const ip_addr_t& addr, const char* name, const Query& q, Context* ctx) {
    ip_addr_t raddr = {}; // Resolved address
    if (q.q.qtype == Type::AAAA && 0) {
...
int Dns::sendErrorResponse(int error, const char* name, const Query& q, Context* ctx) {
    // Allocate a buffer for response data
...
int Dns::getHostByName(const char* name, ip_addr_t* addr, Query* q) {
    const uint8_t addrType = (q->type == Type::A) ? LWIP_DNS_ADDRTYPE_IPV4 : LWIP_DNS_ADDRTYPE_IPV6;
#if !LWIP_IPV6
    if (addrType == LWIP_DNS_ADDRTYPE_IPV6) {
        return SYSTEM_ERROR_NOT_SUPPORTED;
    }
#endif // !LWIP_IPV6
    LwipTcpIpCoreLock lock; // LwIP's DNS client API is not thread-safe
    const auto lwipRet = dns_gethostbyname_addrtype(name, addr, Dns::dnsCallback, q, addrType);
    lock.unlock();
...
void Dns::dnsCallback(const char* name, const ip_addr_t* addr, void* data) {
