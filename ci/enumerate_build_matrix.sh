...
DEBUG_BUILD=( y n )
PLATFORM=( argon boron asom bsom b5som esomx p2 trackerm msom electron2 sulu )
# All modules are now built by release scripts instead, skip
# Only building applications and tests here
# PLATFORM_BOOTLOADER=( argon boron asom bsom b5som tracker esomx p2 trackerm msom electron2 sulu )
...
MODULAR_PLATFORM=( argon boron asom bsom b5som tracker esomx p2 trackerm msom electron2 sulu )
...
do
  ...
  do
    ...
    if [[ "$db" = "y" ]]; then
      if [[ "$p" = "argon" ]] || [[ "$p" = "boron" ]] || [[ "$p" = "asom" ]] || [[ "$p" = "bsom" ]] || [[ "$p" = "b5som" ]] || [[ "$p" = "tracker" ]] || [[ "$p" = "esomx" ]] || [[ "$p" = "p2" ]] || [[ "$p" = "trackerm" ]] || [[ "$p" = "msom" ]] || [[ "$p" = "electron2" ]] || [[ "$p" = "sulu" ]]; then
        continue
      fi
    fi
    cmd="${MAKE} DEBUG_BUILD=\"$db\" PLATFORM=\"$p\" COMPILE_LTO=\"n\""
