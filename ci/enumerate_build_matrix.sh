...
DEBUG_BUILD=( y n )
PLATFORM=( photon p1 electron xenon argon boron bsom b5som )
# P1 bootloader built with gcc 4.8.4 doesn't fit flash, disabling for now
PLATFORM_BOOTLOADER=( photon electron xenon argon boron bsom b5som )
APP=( "" tinker product_id_and_version)
TEST=( wiring/api wiring/no_fixture )

MODULAR_PLATFORM=( photon p1 electron xenon argon boron bsom b5som )
...
do
  ...
  do
    ...
    if [[ "$db" = "y" ]]; then
      if [[ "$p" = "photon" ]] || [[ "$p" = "p1" ]] || [[ "$p" = "xenon" ]] || [[ "$p" = "argon" ]] || [[ "$p" = "boron" ]] || [[ "$p" = "bsom" ]] || [[ "$p" = "b5som" ]]; then
        continue
      fi
    fi
    cmd="${MAKE} DEBUG_BUILD=\"$db\" PLATFORM=\"$p\" COMPILE_LTO=\"n\""
