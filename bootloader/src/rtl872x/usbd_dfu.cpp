...
int DfuClassDriver::dataIn(unsigned ep, particle::usbd::EndpointEvent ev, size_t len) {
  switch (state_) {
    case detail::dfuDNBUSY: {
...
int DfuClassDriver::dataOut(unsigned ep, particle::usbd::EndpointEvent ev, size_t len) {
  return 0;
}
...
int DfuClassDriver::getConfigurationDescriptor(uint8_t* buf, size_t length, Speed speed, unsigned index) {
  unsigned cfgDescLen = std::min<size_t>(length, *((uint16_t*)(cfgDesc_ + 2)));
  if (buf) {
    memcpy(buf, cfgDesc_, cfgDescLen);
  }
  return cfgDescLen;
}

int DfuClassDriver::getString(unsigned id, uint16_t langId, uint8_t* buf, size_t length) {
  if (id < stringBase_) {
    return -1;
  }
  ...
  return dev_->getUnicodeString(s, strlen(s), buf, length);
}

int DfuClassDriver::getNumInterfaces() const {
...
bool DfuClassDriver::registerMal(unsigned index, DfuMal* mal) {
  if (index < HAL_PLATFORM_USB_DFU_INTERFACES) {
    mal_[index] = mal;
