...
SCENARIO("System version info is retrieved", "[system,version]") {
    ...
    if ((SYSTEM_VERSION & 0xFF) == 0xFF) {
        ...
        // FIXME: change back to -rc.X
        // TODO: make it possible to specify anything other than alpha/beta for an RC
        sprintf(expected, "%d.%d.%d-b5som.%d", BYTE_N(info.versionNumber,3), BYTE_N(info.versionNumber,2), BYTE_N(info.versionNumber,1), BYTE_N(info.versionNumber,0) & 0x3F);
