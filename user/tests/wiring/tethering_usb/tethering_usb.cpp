...
test(07_TETHERING_USB_start_iperf_server) {
...
test(08_TETHERING_USB_iperf3_udp_sustain_down) {
}

test(09_TETHERING_USB_iperf3_udp_sustain_down_small) {
}

test(10_TETHERING_USB_iperf3_udp_sustain_up) {
}

test(11_TETHERING_USB_iperf3_udp_sustain_up_small) {
}

test(12_TETHERING_USB_iperf3_tcp_down) {
}

test(13_TETHERING_USB_iperf3_tcp_up) {
}

test(14_TETHERING_USB_stop_iperf_server) {
    LOG(INFO, "Stopping iperf server");
    iperf.stop();
}

test(15_TETHERING_USB_download_speeds) {
}

test(16_TETHERING_USB_upload_speeds) {
}

test(17_TETHERING_USB_iperf3_udp_full_path_down) {
    LOG(INFO, "Device iperf3 UDP full-path down starting");
    String json = runDeviceIperf(/*udp*/true, /*reverse*/true, /*bitrate*/1000000);
    LOG(INFO, "Device iperf3 UDP full-path down done, json=%d", json.length());
    if (json.length() == 0) {
        json = "{}";
    }
...
test(19_TETHERING_USB_iperf3_tcp_full_path_down) {
    LOG(INFO, "Device iperf3 TCP full-path down starting");
    String json = runDeviceIperf(/*udp*/false, /*reverse*/true, /*bitrate*/0);
    LOG(INFO, "Device iperf3 TCP full-path down done, json=%d", json.length());
    if (json.length() == 0) {
        json = "{}";
    }
    pushMailboxMsg(json, 10000);
...
test(20_TETHERING_USB_iperf3_tcp_full_path_up) {
    LOG(INFO, "Device iperf3 TCP full-path up starting");
    String json = runDeviceIperf(/*udp*/false, /*reverse*/false, /*bitrate*/0);
    LOG(INFO, "Device iperf3 TCP full-path up done, json=%d", json.length());
    if (json.length() == 0) {
        json = "{}";
    }
    pushMailboxMsg(json, 10000);
...
test(21_TETHERING_USB_final_report) {
