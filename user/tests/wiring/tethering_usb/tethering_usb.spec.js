...
// Raw tether-link floors, per platform and direction. Empirical: PPP-over-USB-CDC
// is device-CPU-bound well below the ~9.7 Mbps USB full-speed bulk ceiling
// (observed: b5som ~1.45M/0.86M, msom ~2.1M/0.63M down/up).
const LINK_BUDGET_BPS = {
    b5som: { down: 800000, up: 500000 },
    msom: { down: 1200000, up: 400000 },
    default: { down: 500000, up: 300000 },
};
...
test('01_TETHERING_USB_setup_and_bind', async function() {
    ...
    tether = new Tether('usb', device.id, '');
    await tether.start();
});

test('02_TETHERING_USB_resolve_port', async function() {
    tether.port = await tether.resolveTty(device.id);
    console.log(`Resolved port: ${tether.port}`);
});

test('03_TETHERING_USB_configure_port', async function() {
    await tether.setupPort();
});

test('04_TETHERING_USB_wait_for_modem', async function() {
    await tether.waitForModem();
    console.log('Particle modem detected');
});

test('05_TETHERING_USB_wait_for_ppp0', async function() {
    await tether.waitForPpp0();
    console.log('ppp0 is UP');
});

test('06_TETHERING_USB_verify_reachability', async function() {
...
test('07_TETHERING_USB_start_iperf_server', async function() {
    // Device-side starts iperf server
});

test('08_TETHERING_USB_iperf3_udp_sustain_down', async function() {
    // USB CDC full-speed: 64-byte bulk transfers, theoretical max ~9.7 Mbps
    if (!tether.tetherIp) {
...
test('09_TETHERING_USB_iperf3_udp_sustain_down_small', async function() {
...
test('10_TETHERING_USB_iperf3_udp_sustain_up', async function() {
    await tether.ping({ host: tether.tetherIp });
    const result = await tether.iperf3UdpSustain({
...
test('11_TETHERING_USB_iperf3_udp_sustain_up_small', async function() {
    // Small packet size (64 bytes) - real-time/telemetry profile
    await tether.ping({ host: tether.tetherIp });
    const result = await tether.iperf3UdpSustain({
...
test('12_TETHERING_USB_iperf3_tcp_down', async function() {
    await tether.ping({ host: tether.tetherIp });
...
test('13_TETHERING_USB_iperf3_tcp_up', async function() {
    await tether.ping({ host: tether.tetherIp });
...
test('14_TETHERING_USB_stop_iperf_server', async function() {
    // Device-side stops iperf server
});

test('15_TETHERING_USB_download_speeds', async function() {
    await tether.ping({ host: tether.tetherIp });
...
test('16_TETHERING_USB_upload_speeds', async function() {
    await tether.ping({ host: tether.tetherIp });
    const bps = minBps;
...
test('17_TETHERING_USB_iperf3_udp_full_path_down', async function() {
    await tether.ping({ host: tether.tetherIp });

    // Device-side result is pushed to mailbox immediately after device test finishes.
    // Read and print it before running the container-side test.
    expect(device.mailBox).to.not.be.empty;
...
test('18_TETHERING_USB_iperf3_udp_full_path_up', async function() {
    await tether.ping({ host: tether.tetherIp });
...
test('19_TETHERING_USB_iperf3_tcp_full_path_down', async function() {
    await tether.ping({ host: tether.tetherIp });
...
test('20_TETHERING_USB_iperf3_tcp_full_path_up', async function() {
    await tether.ping({ host: tether.tetherIp });
...
test('21_TETHERING_USB_final_report', async function() {
