...
const char eventNameBase[] = "sleep20-test";
...
void resetBleTestPeerTest() {
    detail::gBleTestPeer.test = 0;
}
...
bool commonEventHandler(const char* eventName, const char* data) {
    ...
    while (it.next()) {
        ...
        } else if (it.name() == "test" && it.value().isNumber()) {
            peer.test = it.value().toInt();
        } else if (it.name() == "passkey" && it.value().isString()) {
            passkey.passkey = String(it.value().toString());
        }
    }
    if (peer.address != "" && peer.deviceId != "" && peer.test != -1) {
        gBleTestPeer = peer;
...
bool publishBlePeerInfo(int testNum = 0) {
    using namespace detail;
...
void subscribePeerEvents(hal_ble_role_t role) {
    if (role == BLE_ROLE_CENTRAL) {
        Particle.subscribe(generatePeerEventName(), centralEventHandler);
    } else {
        Particle.subscribe(generatePeerEventName(), peripheralEventHandler);
    }
}
