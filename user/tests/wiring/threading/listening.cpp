...
test(08_THREADING_app_can_exit_listen_mode_via_timeout)
{
    ...
    Particle.process();
    delay(4000);  // todo - find a better way to interact with the system thread
    assertFalse(network_listening());
}
...
test(10_THREADING_app_can_invoke_synchronous_fn_in_listening_mode)
{
	network_listen(true);
...
test(11_THREADING_publish_during_listening_mode_issue_761)
{
	listening_start_with_delay();
