...
#define WIRE Wire  // use D0/D1
// #define WIRE Wire1 // use C4/C5

// Switch SPI interfaces
#define SPI_INTERFACE SPI  // use A2-A5
// #define SPI_INTERFACE SPI1 // use A2 (CS) & D2-D4
...
void LSM9DS1::SPIwriteByte(uint8_t csPin, uint8_t subAddress, uint8_t data)
{
	...
	SPI_INTERFACE.transfer(subAddress & 0x3F); // Send Address
	SPI_INTERFACE.transfer(data); // Send data
							...
							uint8_t * dest, uint8_t count)
{
	...
	digitalWrite(csPin, LOW); // Initiate communication
	SPI_INTERFACE.transfer(rAddress);
	for (int i=0; i<count; i++)
	{
		dest[i] = SPI_INTERFACE.transfer(0x00); // Read into destination array
	}
	digitalWrite(csPin, HIGH); // Close communication
...
{
	WIRE.begin();	// Initialize I2C library on D0/D1
}
