...
test('max_variable_value_size', async function() {
	...
	let maxLen = device.platform.is('gen2') ? 864 : 1024; // See MAX_VARIABLE_VALUE_LENGTH in protocol_defs.h
	const resp = await api.getVariable({ deviceId, name: 'var_s', auth });
	if (device.platform.is('boron') || device.platform.is('bsom')) {
		if (resp.body.result.length < maxLen) {
