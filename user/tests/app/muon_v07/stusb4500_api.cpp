...
void onPdAttachInterrupt(void* context) {
    Log.info(" >>> PD Attach Interrupt");
}

void onPdIntInterrupt(void* context) {
    Log.info(" >>> PD Interrupt");
    stusb4500LogStatus();
}
...
int stusb4500Init(void) {
    ...
    Log.info("Configure pins for STUSB4500.");
    ...
    Mcp23s17::getInstance().attachPinInterrupt(pdAttachPin.first, pdAttachPin.second, FALLING, onPdAttachInterrupt, nullptr);
    Mcp23s17::getInstance().attachPinInterrupt(pdIntPin.first, pdIntPin.second, FALLING, onPdIntInterrupt, nullptr);
