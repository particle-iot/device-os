...
STARTUP (
    System.setPowerConfiguration(SystemPowerConfiguration()
        ...
        .auxiliaryPowerControlPin(PowerModule::auxEnPin())
        .interruptPin(PowerModule::intPin())
...
void setup() {
    ...
    // 10 clock cycles (max. 32), @1Hz
    // You might see unexpected reset if the RTC is powered on without feeding the watchdog
    Am18x5::getInstance().enableWatchdog(30, Am18x5TimerFrequency::HZ_1);
    ...
    if (false) {
        Kg200z::getInstance().uartDownloadMode(true); // Enter download mode, no more logs to UART
    }
...
void loop() {
    ...
    if (millis() - startTime > 3000) {
        ...
        Am18x5::getInstance().feedWatchdog();
    }
}
