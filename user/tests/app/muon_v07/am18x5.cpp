...
int Am18x5::enableWatchdog(uint8_t value, Am18x5TimerFrequency frequency) const {
    Am18x5Lock lock();
    CHECK_TRUE(initialized_, SYSTEM_ERROR_INVALID_STATE);
...
int Am18x5::feedWatchdog() const {
    ...
    if (RTC_WDI != PIN_INVALID) {
        ...
        uint8_t val = 0;
        CHECK(readRegister(Am18x5Register::WDT, &val));
        CHECK(writeRegister(Am18x5Register::WDT, val));
    }
    return SYSTEM_ERROR_NONE;
}
