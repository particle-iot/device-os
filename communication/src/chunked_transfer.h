...
public:
			...
			callback_millis millis)
	{
		...
		size_t size = Messages::coded_ack(response.buf(),
				success ? 0x00 : RESPONSE_CODE(4, 00), queue[2], queue[3]);
		...
		if (success)
		{
			if (!prepare_for_firmware_update(file, 0, NULL))
			{
				...
				size_t size = Messages::update_ready(response.buf(), token,
						(flags & 0x1));
				response.set_length(size);
	...
	template<typename callback_save_firmware_chunk,
			typename callback_calculate_crc,
			typename callback_finish_firmware_update, typename callback_millis>
	ProtocolError handle_chunk(token_t token, Message& message,
			MessageChannel& channel,
			callback_save_firmware_chunk save_firmware_chunk,
			callback_calculate_crc calculate_crc,
			callback_finish_firmware_update finish_firmware_update,
			callback_millis millis)
	{
		last_chunk_millis = millis();
		...
		// send ACK
		uint8_t* queue = message.buf();
		size_t response_size = Messages::empty_ack(response.buf(), queue[2],
				queue[3]);
	...
	ProtocolError handle_update_done(token_t token, Message& message,
			MessageChannel& channel,
			callback_finish_firmware_update finish_firmware_update,
			callback_millis millis)
	{
	...
	ProtocolError send_missing_chunks(Message& message, MessageChannel& channel,
			size_t count)
	{
		size_t sent = 0;
		chunk_index_t idx = 0;
	...
	template<typename callback_finish_firmware_update> void cancel(
			callback_finish_firmware_update finish)
	{
		if (is_updating())
		{
