...
// Message lookup table generated by the `update-msg` tool
const void* _log_message_data __attribute__((weak)) = nullptr;
size_t _log_message_data_size __attribute__((weak)) = 0;
...
// Returns `true` if the message lookup table is available
inline bool hasLookupTable() {
    return (_log_message_data != nullptr);
}

void formatMessage(Print* strm, const char* args, unsigned id) {
    if (!hasLookupTable()) {
        return;
    }
    struct Msg {
    ...
    const Msg* msgs = static_cast<const Msg*>(_log_message_data);
    const size_t msgCount = _log_message_data_size / sizeof(Msg);
    // Find message by ID
    const Msg* m = std::lower_bound(msgs, msgs + msgCount, id, [](const Msg& m, unsigned id) {
        return (m.id < id);
    });
    if (m == msgs + msgCount) {
    ...
    while (*s != '\0') {
        ...
        strm->write((const uint8_t*)s1, s2 - s1);
        if (args) {
            const char* s = strchrnul(args, 0x1f); // Unit Separator (US)
            strm->write((const uint8_t*)args, s - args);
            if (*s != '\0') {
                args = s + 1;
            } else {
                args = nullptr;
    ...
    strm->write((const uint8_t*)s1, s - s1);
}
...
void spark::StreamLogHandler::logMessage(const char *msg, LogLevel level, const char *category, const LogAttributes &attr) {
    ...
    // Message
    if (attr.has_id) {
        formatMessage(stream_, msg, attr.id);
    } else if (msg) {
        write(msg);
    }
