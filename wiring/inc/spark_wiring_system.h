...
/**
 * 1. CPU is stopped
 * 2. Network is stopped
 * 3. Internal/external peripherals are stopped
 * 4. RAM memory is not retained
 */
enum class SystemSleepMode: uint16_t {
    NONE        = SYSTEM_SLEEP_MODE_NONE,
    STOP        = SYSTEM_SLEEP_MODE_STOP,
    HIBERNATE   = SYSTEM_SLEEP_MODE_HIBERNATE,
    SHUTDOWN    = SYSTEM_SLEEP_MODE_SHUTDOWN
};

class SystemSleepConfiguration {
public:
    SystemSleepConfiguration() {
        config_.size = sizeof(hal_core_sleep_config_t);
        config_.version = HAL_CORE_SLEEP_VERSION;
        config_.mode = SYSTEM_SLEEP_MODE_NONE;
    ...
    SystemSleepConfiguration& gpio(pin_t pin, InterruptMode mode) {
        clearGpioWakeupSources();
        return addGpioWakeupSource(pin, mode);
    }

    SystemSleepConfiguration& gpios(std::initializer_list<hal_core_wakeup_source_gpio_t> gpios) {
        clearGpioWakeupSources();
        for (auto gpio : gpios) {
            addGpioWakeupSource(gpio.pin, gpio.mode);
        }
        return *this;
    }

    SystemSleepConfiguration& duration(system_tick_t ms) {
        // Iterates current wakeup sources to check if it has configured RTC wakeup
        auto wakeup = config_.wakeup_sources;
        bool configured = false;
        while (!wakeup) {
            if (wakeup->type == HAL_CORE_WAKEUP_SOURCE_TYPE_RTC) {
                configured = true;
                break;
            }
            wakeup = wakeup->next;
        ...
        if (configured) {
            wakeup->source.rtc.ms = ms;
        } else {
            auto hal_core_wakeup_source_t wakeupSource = new(std::nothrow) hal_core_wakeup_source_t();
            if (!wakeupSource) {
                return *this;
            }
            wakeupSource->size = sizeof(hal_core_wakeup_source_t);
            wakeupSource->version = HAL_CORE_SLEEP_VERSION;
            wakeupSource->type = HAL_CORE_WAKEUP_SOURCE_TYPE_RTC;
            wakeupSource->source.rtc.ms = ms;
    ...
    SystemSleepConfiguration& duration(std::chrono::minutes min) {
        std::chrono::milliseconds ms = std::chrono::duration_cast<std::chrono::milliseconds>(min);
        return duration(ms.count());
    }

    SystemSleepConfiguration& duration(std::chrono::hours h) {
        std::chrono::milliseconds ms = std::chrono::duration_cast<std::chrono::milliseconds>(h);
        return duration(ms.count());
    }

private:
    SystemSleepConfiguration& addGpioWakeupSource(pin_t pin, InterruptMode mode) {
        auto hal_core_wakeup_source_t wakeupSource = new(std::nothrow) hal_core_wakeup_source_t();
        if (!wakeupSource) {
            return *this;
        }
        wakeupSource->size = sizeof(hal_core_wakeup_source_t);
        wakeupSource->version = HAL_CORE_SLEEP_VERSION;
        wakeupSource->type = HAL_CORE_WAKEUP_SOURCE_TYPE_GPIO;
        wakeupSource->source.gpio.pin = pin;
        wakeupSource->source.gpio.mode = mode;
        wakeupSource->next = config_.wakeup_sources;
        config_.wakeup_sources = wakeupSource;
        return *this;
    }

    void clearGpioWakeupSources() {
        auto wakeup = config_.wakeup_sources;
        auto preWakeup = nullptr;
        while (!wakeup) {
            if (wakeup->type == HAL_CORE_WAKEUP_SOURCE_TYPE_GPIO) {
                if (wakeup == config_.wakeup_sources) {
                    // First node
                    config_.wakeup_sources = wakeup->next;
                    delete wakeup;
                    wakeup = config_.wakeup_sources;
                } else {
                    preWakeup->next = wakeup->next;
                    delete wakeup;
                    wakeup = preWakeup->next;
                }
            } else {
                preWakeup = wakeup;
                wakeup = wakeup->next;
    ...
    hal_core_sleep_config_t config_;
};
