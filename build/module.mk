...
st-flash: $(MAKE_DEPENDENCIES) $(TARGET_BASE).bin
	@echo Flashing $(TARGET_BASE).bin using st-flash to address $(PLATFORM_DFU)
	st-flash write $(TARGET_BASE).bin $(PLATFORM_DFU)

ifneq ("$(OPENOCD_HOME)","")

program-openocd: $(MAKE_DEPENDENCIES) $(TARGET_BASE).bin
	@echo Flashing $(TARGET_BASE).bin using openocd to address $(PLATFORM_DFU)
	$(OPENOCD_HOME)/openocd -f $(OPENOCD_HOME)/tcl/interface/ftdi/particle-ftdi.cfg -f $(OPENOCD_HOME)/tcl/target/stm32f2x.cfg  -c "init; reset halt" -c "flash protect 0 0 11 off" -c "program $(TARGET_BASE).bin $(PLATFORM_DFU) reset exit"
...
ifeq ("$(wildcard $(PARTICLE_SERIAL_DEV))","")
	...
	$(DFU) -d $(USBD_VID_SPARK):$(USBD_PID_DFU) -a 0 -s $(PLATFORM_DFU)$(if $(PLATFORM_DFU_LEAVE),:leave) -D $(TARGET_BASE).dfu
...
program-cloud: $(MAKE_DEPENDENCIES) $(TARGET_BASE).bin
	...
	$(CURL) -X PUT -F file=@$(TARGET_BASE).bin -F file_type=binary $(CLOUD_FLASH_URL)
...
# Got some issue currently in getting 'sz' working
	sz -b -v --ymodem $(TARGET_BASE).bin > $(PARTICLE_SERIAL_DEV) < $(PARTICLE_SERIAL_DEV)
endif
endif
